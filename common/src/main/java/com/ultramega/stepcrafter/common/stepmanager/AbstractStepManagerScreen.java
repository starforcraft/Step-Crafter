...
public abstract class AbstractStepManagerScreen<T extends AbstractStepManagerContainerMenu> extends AbstractStretchingScreen<T>
    ...
    public void extractRenderState(final GuiGraphicsExtractor graphics, final int mouseX, final int mouseY, final float partialTicks) {
        ...
        if (this.searchField != null) {
            this.searchField.extractRenderState(graphics, 0, 0, 0);
        }
    }
    ...
    private void renderGroups(final GuiGraphicsExtractor graphics,
        ...
        for (final AbstractStepManagerContainerMenu.ViewGroup group : this.menu.getGroups()) {
            ...
            if (!isOutOfFrame(y, topHeight, rows, rowY)) {
                ...
                graphics.text(this.font, group.getName(), rowX + 4, rowY + 6, -12566464, false);
            }
    ...
    protected void extractTooltip(final GuiGraphicsExtractor graphics, final int x, final int y) {
        if (this.tooltip != null && !this.tooltip.isEmpty()) {
            graphics.tooltip(this.font, this.tooltip, x, y, DefaultTooltipPositioner.INSTANCE, null);
            this.tooltip = null;
            return;
        }
    ...
    @Override
    protected void renderStretchingBackground(final GuiGraphicsExtractor graphics, final int x, final int y, final int rows) {
        for (int row = 0; row < rows; ++row) {
            ...
            graphics.blit(GUI_TEXTURED, this.getTexture(), x, y + (ROW_SIZE * row), 0, textureY, this.imageWidth, ROW_SIZE, 256, 256);
        }
    }
